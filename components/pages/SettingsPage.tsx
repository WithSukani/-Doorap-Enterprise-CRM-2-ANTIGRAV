...
interface SettingsPageProps {
    ...
    setIntegrationSettings?: (settings: any[]) => void;
}
...
const SettingsPage: React.FC<SettingsPageProps> = ({
    ...
    // Portal State - Sync with Props
    const rightmoveSetting = portalSettings.find(p => p.portalName === 'rightmove');
    const zooplaSetting = portalSettings.find(p => p.portalName === 'zoopla');

    const [rightmoveEnabled, setRightmoveEnabled] = useState(rightmoveSetting?.isActive || false);
    const [zooplaEnabled, setZooplaEnabled] = useState(zooplaSetting?.isActive || false);
                    ...
                    <nav className="flex flex-col p-2 space-y-1">
                        {tabs.map(tab => (
                            <button
                                ...
                                className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors ${activeTab === tab.id ? 'bg-zinc-900 text-white' : 'text-zinc-600 hover:bg-zinc-100'
                                    }`}
                            >
            ...
            <div className="flex-1">
                ...
                {activeTab === 'integrations' && (
                    <>
                        ...
                        <div className="bg-white p-6 rounded-lg border border-zinc-200 shadow-sm max-w-2xl mt-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-lg font-semibold text-zinc-900">Gemini AI Integration</h3>
                                    <p className="text-sm text-zinc-500">Power "Ask Dori" and automated insights with Google Gemini.</p>
                                </div>
                                <span className="px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full flex items-center"><CheckCircleIcon className="w-3 h-3 mr-1" /> Active (Universal)</span>
                            </div>
                        ...
                        <div className="bg-white p-6 rounded-lg border border-zinc-200 shadow-sm max-w-2xl mt-6">
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-lg font-semibold text-zinc-900">Voice AI & Telephony</h3>
                                    <p className="text-sm text-zinc-500">Connect Twilio and ElevenLabs to power Dori's voice capabilities.</p>
                                </div>
                                {integrationSettings?.find(s => s.provider === 'twilio')?.isActive && (
                            ...
                            <div className="text-sm text-zinc-600 bg-zinc-50 p-4 rounded-md border border-zinc-200">
                                <p className="mb-2"><strong><CheckCircleIcon className="w-4 h-4 inline mr-1 text-green-600" /> System Optimized</strong></p>
                                <p>Your dedicated Dori Voice Agent is fully configured and managed by the Doorap platform. No manual configuration is required.</p>
                                {integrationSettings?.find(s => s.provider === 'twilio')?.phoneNumber && (
                                    <p className="mt-2 text-xs text-zinc-500 font-mono">
                                        Assigned Line: {integrationSettings.find(s => s.provider === 'twilio')?.phoneNumber}
                                    </p>
                                )}
                            </div>
                        </div>
